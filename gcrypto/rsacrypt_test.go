...
var (
	rsaMsg           = "床前明月光，疑是地上霜，举头望明月，低头思故乡"
...
func TestRsaEncrypt(t *testing.T) {
	// 公钥加密,私钥解密
	t.Log("*******************TestRsaEncryptBase64***********************")
	base64Key1024, _ := GenerateRsaKeyBase64(1024)
	base64Key2048, _ := GenerateRsaKeyBase64(2048)

	base64CipherText1, _ := RsaEncryptToBase64([]byte(rsaMsg), base64Key1024.PublicKey)
	t.Log("自动公钥1024加密后(base64)：" + base64CipherText1)
	plainText1, _ := RsaDecryptByBase64(base64CipherText1, base64Key1024.PrivateKey)
	t.Log("base64密文自动私钥1024解密：" + string(plainText1))
	// good
	base64CipherText2, _ := RsaEncryptToBase64([]byte(rsaMsg), base64Key2048.PublicKey)
	t.Log("自动公钥2048加密后(base64)：" + base64CipherText2)
	plainText2, _ := RsaDecryptByBase64(base64CipherText2, base64Key2048.PrivateKey)
	t.Log("base64密文自动私钥2048解密：" + string(plainText2))

	// good
	base64CipherText3, _ := RsaEncryptToBase64([]byte(rsaMsg), base64PubKey1024)
	t.Log("手动公钥1024加密后(base64)：" + base64CipherText3)
	plainText3, _ := RsaDecryptByBase64(base64CipherText3, base64PriKey1024)
	t.Log("base64密文手动私钥1024解密：" + string(plainText3))

	t.Log("*******************TestRsaEncryptHex***********************")
	hexKey1024, _ := GenerateRsaKeyHex(1024)
	hexKey2048, _ := GenerateRsaKeyHex(2048)

	hexCipherText1, _ := RsaEncryptToHex([]byte(rsaMsg), hexKey1024.PublicKey)
	t.Log("自动公钥1024加密后(hex)：" + hexCipherText1)
	plainText4, _ := RsaDecryptByHex(hexCipherText1, hexKey1024.PrivateKey)
	t.Log("hex密文自动私钥1024解密：" + string(plainText4))
	// good
	hexCipherText2, _ := RsaEncryptToHex([]byte(rsaMsg), hexKey2048.PublicKey)
	t.Log("自动公钥2048加密后(hex)：" + hexCipherText2)
	plainText5, _ := RsaDecryptByHex(hexCipherText2, hexKey2048.PrivateKey)
	t.Log("hex密文自动私钥2048解密：" + string(plainText5))

	// good
	hexCipherText3, _ := RsaEncryptToHex([]byte(rsaMsg), hexPubKey1024)
	t.Log("手动公钥1024加密后(hex)：" + hexCipherText3)
	plainText6, _ := RsaDecryptByHex(hexCipherText3, hexPriKey1024)
	t.Log("hex密文手动私钥1024解密：" + string(plainText6))
}

func TestRsaSign(t *testing.T) {
	// 私钥签名,公钥验签
	t.Log("*******************TestRsaSignBase64***********************")
	base64Key1024, _ := GenerateRsaKeyBase64(1024)
	...
	t.Logf("私钥1024签名(base64)：%s", base64Sign1024)
	res := RsaVerifySignBase64([]byte(rsaMsg), base64Sign1024, base64Key1024.PublicKey)
	t.Logf("公钥1024验签base64结果：%v", res)

	t.Log("*******************TestRsaSignHex***********************")
	hexKey1024, _ := GenerateRsaKeyHex(1024)
