...
import (
	"github.com/lytdev/go-mykit/glist"
	"math/rand"
	"sync"
	"time"
)
...
func DispatchingStrategyLeast[T any](msg T, index uint64, channels []<-chan T) int {
	seq := glist.Range(len(channels))

	return glist.MinBy(seq, func(item int, min int) bool {
...
func DispatchingStrategyMost[T any](msg T, index uint64, channels []<-chan T) int {
	...
	return glist.MaxBy(seq, func(item int, max int) bool {
